/**
 * Lighter liquidation price estimation.
 *
 * Lighter publishes a flat per-market `maintenance_margin_fraction` on its
 * order books, surfaced on the generic {@link PerpsMarket} as
 * `maintenanceMarginRate`. The estimate is the standard isolated-margin
 * model parameterised by that rate.
 */
...
/**
 * Estimate the liquidation price of a new isolated position on a Lighter
 * market.
 *
