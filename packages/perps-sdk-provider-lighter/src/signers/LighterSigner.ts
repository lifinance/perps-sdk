...
/**
 * Signed TRANSFER result, including the EIP-191 L1 message required to
 * authorize a cross-account transfer.
 *
 * @internal
 */
export interface TransferResult extends LighterSignedBlob {
  /** EIP-191 `Transfer` L1 body the user's wallet must countersign to bind the destination account and the amount to the account owner. */
  messageToSign: string
}
...
export class LighterSigner {
   ...
   *
   * For REGISTER_API_KEY use `signChangePubKey`, for APPROVE_INTEGRATOR use
   * `signApproveIntegrator`, and for TRANSFER use `signTransfer` — all three
   * return an additional `messageToSign` the L1 wallet must countersign.
   */
  async sign(
    action: ActionType,
  ...
  /**
   * Sign a TRANSFER action with the stored API key and return the signed blob
   * alongside the EIP-191 `Transfer` L1 message the user's Ethereum wallet
   * must countersign. Unlike {@link sign}, this exposes `messageToSign`: the
   * L1 signature (injected via {@link embedL1Signature} before submission)
   * binds the destination account and the amount to the account owner, so the
   * API key alone cannot move funds to an address the owner never approved.
   */
  async signTransfer(
    wasmSignParams: Record<string, unknown>,
    context: LighterSignerContext
   ...
   * by the REGISTER_API_KEY (ChangePubKey), APPROVE_INTEGRATOR, and TRANSFER
   * flows; SEND_ASSET stays on the bare signer call and keeps `L1Sig` empty.
   */
  ...
  private dispatch(
    ...
    switch (action) {
      ...
      case ActionType.TRANSFER:
        // A cross-account move: the caller MUST countersign the returned
        // `messageToSign` and inject the result as `L1Sig`, because the memo
        // carries the destination L1 address and only that signature binds it
        // to the account owner. SignTransfer's positional args follow the Go
        // binding order: `toAccountIndex, assetIndex, fromRouteType,
        // toRouteType, amount, usdcFee, memo, skipNonce, nonce, apiKeyIndex,
        // accountIndex`. `memo` is decoded as a 66-char `0x`-prefixed hex
        // string, a bare 64-char hex string, or 32 raw bytes — anything else
        // fails with "memo expected to be 32 bytes or 64 hex encoded or 66 if
        // 0x hex encoded".
      ...
      case ActionType.SEND_ASSET: {
        ...
        // routes, so no L1 signature binds it and `L1Sig` stays empty.
