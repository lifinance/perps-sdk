...
- [#481](https://github.com/lifinance/perps-sdk/pull/481) [`7cf4aa1`](https://github.com/lifinance/perps-sdk/commit/7cf4aa11b500d76e42406696ed353b8833d5f711) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Resolve the Lighter `marketContext` stats channel from the market's category instead of its numeric market id, so a market in Lighter's new id range reaches the correct channel. The subscription now syncs the market registry first and throws `MarketNotFound` when the backend market list does not carry the market id, or when the provider was constructed without a `PerpsSDKClient`.
...
- [#483](https://github.com/lifinance/perps-sdk/pull/483) [`29c2cad`](https://github.com/lifinance/perps-sdk/commit/29c2cad554695923b4abd6222b0437bcbcded17a) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Map the Lighter `pending` and `in-progress` order statuses to `PENDING`. The order mapper threw on both, which made the WebSocket handler discard every order in an `account_all_orders` message.
...
- [#467](https://github.com/lifinance/perps-sdk/pull/467) [`a5dca06`](https://github.com/lifinance/perps-sdk/commit/a5dca069f97ee8d7818b33ea6f8f3acde3c7df03) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Add the optional `price` field to `Balance`. It is the USD price of one unit, as a decimal string. A consumer renders an asset price with no market lookup and no math. The field is absent when the provider holds no price for the asset.
...
- [#467](https://github.com/lifinance/perps-sdk/pull/467) [`1d78d2b`](https://github.com/lifinance/perps-sdk/commit/1d78d2ba44c4339cb9cd18652eb87ee275ccb732) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Rebuild the vendored Lighter WASM signer from lighter-go `v1.0.10`.
...
- [#468](https://github.com/lifinance/perps-sdk/pull/468) [`5a800cc`](https://github.com/lifinance/perps-sdk/commit/5a800cc50843ad0b49c44f50c3f190e601236b88) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Lighter `account_type` is the account's `SubAccountType`, not an account tier. The account-config projection no longer decodes that integer into a tier, so a sub-account no longer reports the `premium` tier. Without a `user_tier_name` from `/accountLimits` the projected tier is now `null`.

- [#467](https://github.com/lifinance/perps-sdk/pull/467) [`9a0b80e`](https://github.com/lifinance/perps-sdk/commit/9a0b80e65c86576b4de577b80ee16f1a43b8a5e4) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Drop a Lighter order row whose market the registry cannot resolve, instead of rejecting the whole order page. One delisted market that the backend market list omits no longer empties the order history. The registry warns once per unresolved id. A row whose market is delisted still resolves, so it stays in the response.

- [#478](https://github.com/lifinance/perps-sdk/pull/478) [`7dab6c7`](https://github.com/lifinance/perps-sdk/commit/7dab6c7c5c958e704daec3c94cc6feb9d3e3f8d7) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Document `LtSendTxRequest` and `LtSendTxBatchRequest` against their Lighter spec schemas and name the generated form-parameter counterparts in the emitted declarations.

- Updated dependencies [[`a5dca06`](https://github.com/lifinance/perps-sdk/commit/a5dca069f97ee8d7818b33ea6f8f3acde3c7df03), [`ef23ec2`](https://github.com/lifinance/perps-sdk/commit/ef23ec23035d809a8e33e27a5158fbb862e11d8b), [`0c7f1b1`](https://github.com/lifinance/perps-sdk/commit/0c7f1b1381b7afd77808eb0558d8a09fed2bb6e1), [`a9853c6`](https://github.com/lifinance/perps-sdk/commit/a9853c622537e1f89690042b5e798b9dd72600e0), [`5a800cc`](https://github.com/lifinance/perps-sdk/commit/5a800cc50843ad0b49c44f50c3f190e601236b88), [`46fd6b5`](https://github.com/lifinance/perps-sdk/commit/46fd6b5dac0d0023185eb6a41db7dab9134eee24)]:
  - @lifi/perps-types@13.0.0
  - @lifi/perps-sdk@14.0.0
...
- Updated dependencies [[`28b4a84`](https://github.com/lifinance/perps-sdk/commit/28b4a841b4c54e7f35a313c8a1726391d3fafaa3)]:
  - @lifi/perps-types@12.0.0
  - @lifi/perps-sdk@11.0.0
...
- [#417](https://github.com/lifinance/perps-sdk/pull/417) [`3472180`](https://github.com/lifinance/perps-sdk/commit/34721809db51640a796a07908cbe10c96f912cd7) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Sequence-gate the internal setup drain and return a renderable setup checklist.
...
- Updated dependencies [[`e6f120b`](https://github.com/lifinance/perps-sdk/commit/e6f120b3aabb0dc1814b125f1fd5ee882c6f0945)]:
  - @lifi/perps-types@11.4.0
...
- [#382](https://github.com/lifinance/perps-sdk/pull/382) [`c72bc0b`](https://github.com/lifinance/perps-sdk/commit/c72bc0b6b315d5b5b15d4bd457cf64c6f6e70011) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - `LtLiquidation` now declares the `trade` and `info` payload that Lighter's `/api/v1/liquidations` returns, and types `type` as the documented `'partial' | 'deleverage'` union instead of `string`. The Lighter liquidation activity therefore reports `liquidatedNotionalPosition` (the forced trade's price times its size), `accountValue` (the pre-trade account value, present only when Lighter reports the cross risk snapshot), `leverageType` (`cross` or `isolated`, read from the row's own position margin mode; an unknown mode reports no `leverageType`), and the liquidated position `size`. A consumer that constructs an `LtLiquidation` must supply the two new members.
...
- [#373](https://github.com/lifinance/perps-sdk/pull/373) [`e0d34a1`](https://github.com/lifinance/perps-sdk/commit/e0d34a17de3db5b8b0c2d0b0bd5b0da0db6f9812) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Lighter `getActivity` now drops a funding or liquidation row whose market the backend market list does not hold, instead of rejecting the whole activity feed.

- [#375](https://github.com/lifinance/perps-sdk/pull/375) [`4d148cd`](https://github.com/lifinance/perps-sdk/commit/4d148cdcbd10a727cc7ef6926fff76fc233a8d65) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Lighter `getFills` now drops a trade row whose market the backend market list does not hold, instead of rejecting the whole fills page.
