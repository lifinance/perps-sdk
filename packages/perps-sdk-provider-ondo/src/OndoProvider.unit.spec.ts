...
const BALANCE_RESULT: OnBalanceSummary = {
  walletBalance: '1000',
  realizedPnl: '25',
  unrealizedPnl: '15.5',
...
const POSITION_RESULT: OnPosition = {
  market: 'AAPL-USD.P',
  direction: 'long',
  netQuantity: '10',
...
const ORDER_UNTRIGGERED: OnOrder = {
  orderId: 'ord-2',
  side: 'sell',
  price: '0',
  size: '10',
...
const FUNDING_RESULT: OnFundingFeeTransfer = {
  market: 'AAPL-USD.P',
...
const LIQUIDATION_RESULT: OnLiquidationEvent = {
  id: 'liq-1',
...
describe('OndoProvider — logged-out degrade paths', () => {
  it('getAccount returns an empty snapshot with a loggedIn: false config', async () => {
    ...
    expect(account).toEqual({
      ...
      config: { provider: 'ondo', loggedIn: false },
    })
...
describe('OndoProvider — getAccountSummary', () => {
  it('treats collateral rows as gross (locked margin included, uPnL from positions)', () => {
    ...
    const account = {
      ...
      unrealizedPnl: '15.5',
      feeTier: { maker: '0.0002', taker: '0.0005' },
      config: { provider: 'ondo', loggedIn: true } as const,
