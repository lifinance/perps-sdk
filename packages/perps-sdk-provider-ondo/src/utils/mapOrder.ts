...
import type { OnOrder, OnOrderStatus, OnOrderType } from '../types/wire.js'

const toIso = (time: string): string => new Date(time).toISOString()

const mapSide = (side: OnOrder['side']): OrderSide =>
  side === 'buy' ? OrderSide.BUY : OrderSide.SELL

/** @public */
export const mapOrderType = (type: OnOrderType): OrderType => {
  switch (type) {
    case 'limit':
 ...
 * @public
 */
export const mapOrderStatus = (status: OnOrderStatus): OrderStatus => {
  switch (status) {
    case 'open':
    case 'untriggered':
 ...
 * @public
 */
export const mapStatusReason = (order: OnOrder): string | undefined => {
 ...
 * @public
 */
export const isTriggerOrder = (order: OnOrder): boolean =>
...
export const mapOrder = (order: OnOrder, market: MarketDisplay): OpenOrder => ({
...
const TERMINAL_STATUSES: ReadonlySet<OnOrderStatus> = new Set([
  'canceled',
  'fullyfilled',
])
