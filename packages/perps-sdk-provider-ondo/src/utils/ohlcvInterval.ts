...
export const intervalFromBarSpan = (
  ...
  const fixed = INTERVAL_SECONDS[spanSecs]
  if (fixed !== undefined) {
    return fixed
  }
  // Monthly bars span a calendar month, so their second-count varies by month.
  if (spanSecs >= MONTH_MIN_SECS && spanSecs <= MONTH_MAX_SECS) {
    return '1M'
  }
  return undefined
}
