...
import type { OnAuthToken, OnGenericResponse } from '../types/auth.js'
import {
  OndoApiClient,
  OndoApiError,
...
const envelope = <T>(result: T): OnGenericResponse<T> => ({
  success: true,
  result,
})
...
describe('OndoApiClient', () => {
  it('unwraps the GenericResponse envelope on GET', async () => {
    ...
    const result = await client.get<OnAuthToken>('/v1/auth/whoami')
  ...
  it('POSTs a JSON body with content-type and unwraps the result', async () => {
    ...
    const result = await client.post<OnAuthToken>(
