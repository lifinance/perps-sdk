...
export const ondoProvider = (
  ...
  return {
    ...
    async getAccount(
      ...
      if (token === null) {
        return loggedOutAccount(params.address)
      }

      const client = apiClient(opts)
      const registry = marketRegistry()
      let balance: OndoBalanceSummary
      let rawPositions: OndoPosition[]
      let referral: OndoAccountReferral | null | undefined
      try {
        ;[balance, rawPositions, referral] = await Promise.all([
    ...
    async getActivity(
      ...
      const [fundings, liquidations] = await Promise.all([
        shouldFetch(ActivityType.FUNDING, 'fundings')
          ? client.getPage<OndoFundingFeeTransfer>('/v1/perps/funding_fees', {
