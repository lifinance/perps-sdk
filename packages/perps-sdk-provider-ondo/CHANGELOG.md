...
- [#467](https://github.com/lifinance/perps-sdk/pull/467) [`fbbbadd`](https://github.com/lifinance/perps-sdk/commit/fbbbadd16986520e9152f701c312aa650458cc79) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - An order row the mapper rejects, such as a lifecycle state the SDK does not carry, is now dropped instead of failing the whole read. `getOrders` warns once per distinct message, and the WebSocket order handler drops the row from the frame. A WebSocket order update whose market the registry cannot resolve now retires its own active entry when the row is terminal.

- [#467](https://github.com/lifinance/perps-sdk/pull/467) [`fcd01f6`](https://github.com/lifinance/perps-sdk/commit/fcd01f63e68ded24348be57c4293769af57029bc) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - `getOrders` sends `activeOnly=true` for active-only reads and no `status` query for reads that include a terminal status, because the server rejects a `status` query. The SDK still filters the rows by the requested statuses. The order cursor now names the regular order source `active` or `all`; a cursor from a previous version is rejected as malformed.
...
- [#414](https://github.com/lifinance/perps-sdk/pull/414) [`1de3304`](https://github.com/lifinance/perps-sdk/commit/1de3304ce4b8fadf807be480a354b12d3116d3a1) Thanks [@aaronmboyd](https://github.com/aaronmboyd)! - Reclaim a stale LI.FI-owned Ondo API key when an account reaches the key limit.
