...
login: the backend issues the challenge, the user signs it with their wallet,
and this package completes the login directly against Ondo — the JWT never
transits the LI.FI backend and is persisted browser-side only.
...
- `completeSiweLogin` — signs a SIWE challenge with the user's wallet and
  exchanges it for an Ondo session token (`OndoAuthToken`).
